...
layui.define(["element", "jquery"], function (exports) {
  ...
  Tab.prototype.navBar = function (strData) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      if (data[i].children != undefined && data[i].children.length > 0) {
        ...
        ulHtml += '<span><cite>' + data[i].name + '</cite></span>';
        ulHtml += '<span class="layui-nav-more"></span>';
        ulHtml += '</a>';
        ulHtml += '<dl class="layui-nav-child">';
        for (var j = 0; j < data[i].children.length; j++) {
          if (data[i].children[j].target == "_blank") {
            ulHtml += '<dd><a href="javascript:void(0);" data-url="' + data[i].children[j].url + '" target="' + data[i].children[j].target + '">';
          } else {
            ulHtml += '<dd><a href="javascript:void(0);" data-url="' + data[i].children[j].url + '">';
          }
          ...
          ulHtml += '<span><cite>' + data[i].children[j].name + '</cite></span></a></dd>';
        }
        ulHtml += "</dl>";
      } else {
        if (data[i].target == "_blank") {
          ulHtml += '<a href="javascript:void(0);" data-url="' + data[i].url + '" target="' + data[i].target + '">';
        } else {
          ulHtml += '<a href="javascript:void(0);" data-url="' + data[i].url + '">';
        }
        if (data[i].icon != undefined && data[i].icon != '') {
          if (data[i].icon.indexOf("icon-") != -1) {
            ulHtml += '<i class="seraph ' + data[i].icon + '" data-icon="' + data[i].icon + '"></i>';
        ...
        ulHtml += '<span><cite>' + data[i].name + '</cite></span></a>';
      }
      ulHtml += '</li>';
    }
    return ulHtml;
  ...
  Tab.prototype.render = function () {
    //显示左侧菜单
    var _this = this;
    $(".navBar ul").html('<li class="layui-nav-item layui-this"><a data-url="page/main.html" href="javascript:void(0);"><i class="layui-icon" data-icon=""></i><span><cite>首页</cite></span></a></li>').append(_this.navBar(menus)).height($(window).height() - 210);
